...
Follow `.ai-setup/onboard.md`: explore the repo and write the project-context sections (project
structure, weighted features, conventions that differ from stack norms, and Operations) into
`AGENTS.md`, above the `<!-- ai-setup >>>` managed block. Leave that block untouched.

## 2. Tailor the pre-commit checks

Using the Operations context you just captured, update `.ai-setup/precommit` to run the
...
Summarize: what context you captured, what you changed in `.ai-setup/precommit` and
