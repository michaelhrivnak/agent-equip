...
test("forked target (edited since last write) is left untouched, hash preserved", () => {
	...
	expect(existsSync(`${dst}.ai-setup-new`)).toBe(false); // silent, no churn
	expect(r.hash).toBe(hash("v1\n")); // baseline kept
});

test("pre-existing foreign file is never overwritten; leaves a *.ai-setup-new and records the fork sentinel", () => {
	const dst = join(ctx.target, "out");
	...
	expect(readFileSync(`${dst}.ai-setup-new`, "utf8")).toBe("v1\n");
...
test("with the fork sentinel recorded, a later run is silent and does not recreate the artifact", () => {
	...
	expect(existsSync(`${dst}.ai-setup-new`)).toBe(true);

	rmSync(`${dst}.ai-setup-new`); // user reviews and removes the reconcile copy
	...
	expect(existsSync(`${dst}.ai-setup-new`)).toBe(false); // NOT recreated
});
